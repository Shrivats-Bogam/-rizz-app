...
API_KEY = os.getenv("API_KEY", "rizz-default-key")
...
async def generate_rizz(request: GenerateRequest, x_api_key: str = Header(None)):
...
async def rewrite_rizz(request: RewriteRequest, x_api_key: str = Header(None)):
    verify_api_key(x_api_key)
    try:
...
async def analyze_chat(request: AnalyzeRequest, x_api_key: str = Header(None)):
    verify_api_key(x_api_key)
    try:
